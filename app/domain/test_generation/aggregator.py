...
class TestAggregator:
    ...
    def aggregate_results(
        ...
        logger.info("Aggregated %d sections from %d results", 
                   len(section_tests), len(results))
    ...
    def _merge_section_tests(
        ...
        merged = self._reconstruct_test(settings, variables, all_test_cases, all_keywords)
        return merged
    ...
    def _combine_sections(self, section_tests: dict[str, str]) -> str:
        ...
        section_order = [
            "header", "navigation", "main_content", "forms",
            "sidebar", "footer", "modal", "unknown"
        ]
        ...
        for section_type in section_order:
            ...
            tc = [f"Test {section_type.title()} - {name.strip()}" 
