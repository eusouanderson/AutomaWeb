...
class DOMPreprocessor:
    ...
    INTERACTIVE_TAGS = {
        ...
        "label", "option", "fieldset", "legend", "optgroup"
    }
    
    # Container tags that group content
    CONTAINER_TAGS = {
        "div", "section", "article", "aside", "main", "nav",
        "header", "footer", "body", "html", "span"
    }
    
    # Tags to completely skip
    SKIP_TAGS = {
        "script", "style", "meta", "link", "noscript", "iframe",
        "svg", "path", "defs", "symbol", "use", "image"
    }
    
    # Attributes to preserve (all others will be dropped)
    PRESERVE_ATTRIBUTES = {
        "id", "name", "role", "aria-label", "aria-labelledby",
        "aria-describedby", "aria-expanded", "aria-hidden",
        "href", "type", "placeholder", "onclick", "class",
        "data-testid", "value", "checked", "selected", "disabled",
        "required", "readonly", "autocomplete", "maxlength",
        "pattern", "min", "max", "step", "for", "tabindex"
    }
    ...
    def _clean_attribute_value(self, attr_name: str, attr_value: Any) -> str | None:
        ...
        if len(value_str) > max_length:
            ...
            if attr_name == "class":
                ...
                key_classes = [c for c in classes 
                             if not re.match(r'^(m-|p-|ml-|mr-|mt-|mb-|pl-|pr-|pt-|pb-|w-|h-|gap-|d-)', c)]
                if key_classes:
    ...
    def create_processed_element(self, element_dict: dict) -> ProcessedElement:
        ...
        return ProcessedElement(
            ...
            children=[self.create_processed_element(child) 
