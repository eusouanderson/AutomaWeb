...
from app.domain.dom.models import DOMSection, DOMChunk, ProcessedElement, DOMSectionType
...
class DOMChunker:
    ...
    def chunk_section(self, section: DOMSection) -> list[DOMChunk]:
        ...
        for elem in section.elements:
            ...
            if (
                current_elements
                and (current_size + elem_size) > self.target_chunk_chars
            ):
    ...
    def chunk_all_sections(self, sections: list[DOMSection]) -> list[DOMChunk]:
        ...
        logger.info(
            "Created %d total chunks from %d sections", len(all_chunks), len(sections)
        )
    ...
    def is_within_token_limit(self, char_size: int, token_limit: int) -> bool:
...
class AdaptiveChunker(DOMChunker):
    ...
    def __init__(
        self, total_token_budget: int, strategy: ChunkingStrategy | None = None
    ):
    ...
    def chunk_all_sections_adaptive(self, sections: list[DOMSection]) -> list[DOMChunk]:
